...
Gilded Weaver doesn't need to breach Task Force Nightfall's networks directly — they just need to be there when the firmware updates arrive. A Meridian Systems Group update handler deployed across the relay network has a quiet anomaly in its chunk parser: it trusts what the packet says, not what it has room for. Someone knew exactly where to put the pressure.
