...
Halcyon has routed its command-signing traffic through a quantum repeater called **NECRO-NODE**  a windowless pod on a high-desert mesa outside Flagstaff. GLASSWING is already inside. Your implant has hooked into the pod's photonic buffer, and the repeater's sync pulses are leaking on the console like whispers in a draft. A Phantom Qubit waits in your register. When the repeater fires its Bell-state measurement, your ghost climbs into the swap and sits down. The Command Centre will reach for the HSM and find only cold air. You do not sniff. You **haunt** .
