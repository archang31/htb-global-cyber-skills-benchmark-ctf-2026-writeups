...
Three days before the election, the coalition's switching fabric started reporting itself as degraded. Anika doesn't believe it — the topology is the same it's been for months, the hardware is healthy, the upstream telemetry is clean. Something inside the management platform is lying about its own state, and whatever it is, it was put there by someone who knew the code would be reviewed in a hurry. Find the thread, follow it back to the hand that planted it.
