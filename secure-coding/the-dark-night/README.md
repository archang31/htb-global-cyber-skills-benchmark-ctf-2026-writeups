...
A forged administrator token surfaced in ORION SENTINEL's audit log last night and nobody on the analyst floor can explain it. Sana has been quietly pulling at the threads of the in-house signing library ever since — the one a contractor wrote in a sprint two years ago and labelled "internal use only." The comments in the source talk about safety. The math underneath the comments tells a different story. Find the gap between what the library claims to do and what it actually does before Vane's people find it first.
