...
The transit cluster's inner gate uses the same token family, but the architecture has been tightened. Sessions are capped, and the signing ritual now includes an extra step. The margin for exploration is thinner than it looks.
